...
spec = do
  ...
  describe "stepFrame" $ do
    it "should have left over delta seconds and set the frame completion flag" $ do
      let delta = 0.9
      let actual = stepFrame Frame { _fLocation = 'a', _fSeconds = 1.0 } Position { _pKeyFrame = (0 :: Int), _pFrameIndex = 0, _pCounter = 0.3 } delta
  ...
  describe "stepWithLoop" $ do
    let getFrames KeyFrame0Stand = [Frame 'a' 0.2, Frame 'b' 0.2]
        getFrames KeyFrame0Walk = [Frame 'c' 0.2, Frame 'd' 0.2]
    let kfs = keyFrames getFrames
    let p = Position { _pKeyFrame = KeyFrame0Stand, _pFrameIndex = 0, _pCounter = 0 }
    it "should do nothing if given 0 delta seconds" $ do
      stepWithLoop kfs p 0 `shouldBe` p
    it "should go to the next frame" $ do
      stepWithLoop kfs p 0.2 `shouldBe` p { _pFrameIndex = 1, _pCounter = 0 }
    it "should loop to the start" $ do
      stepWithLoop kfs p 0.4 `shouldBe` p { _pFrameIndex = 0, _pCounter = 0 }
